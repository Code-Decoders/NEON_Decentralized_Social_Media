App = {
  loading: false,
  ipfs: window.IpfsApi("localhost", 5001, { protocol: "http" }),
  contracts: {},
  load: async () => {
    await App.loadWeb3();
  ...
  loadWeb3: async () => {
    if (window.ethereum) {
      ...
      try {
        ...
        $("#generalModalMessage").text(
          "Permission Denied, Metamask Not connected!"
        );
      }
    } else {
      console.log(
      ...
      $("#generalMsgModal").modal("show");
      $("#generalModalMessage").html(
        "Non-Ethereum browser detected. You should consider trying MetaMask! <br> <a href='https://chrome.google.com/webstore/detail/metamask/nkbihfbeogaeaoehlefnkodbefgpgknn?hl=en'>Download Here</a>"
      );
    }
  },
  ...
  loadContract: async () => {
    ...
    App.NeonLabs = "0xC5E47C64c30c82f2Fb768e0C73614b0813aDeD23";

    if (App.network == "private")
      App.contracts.dwitter = new web3.eth.Contract(abi, App.NeonLabs);
    else App.showError("Please Connect to valid network");
    console.log(App.contracts.dwitter);
  },

  loadUserProfile: async () => {
    App.userStatus = await App.contracts.dwitter.methods
      .userStatus()
    ...
    if (App.userStatus == 0) {
      ...
      $("#registerBtn").on("click", async () => {
        ...
        reader1.onloadend = async function () {
          ...
          reader2.onloadend = async function () {
            ...
            await App.contracts.dwitter.methods
              .registerUser(
                $("#username").val(),
                $("#name").val(),
                result1[0].hash,
      ...
      App.showError(
        "Your Account Has been Banned due Violations of the Platform"
      );
    } else if (App.userStatus == 3) {
      App.showError("Your Account Has been Deleted");
    } else {
      App.user = await App.contracts.dwitter.methods
        .getUser()
        .call({ from: App.account });
      ...
      $("#userProfileImage").css(
        "background-image",
        "url( http://localhost:8080/ipfs/" + App.user.imghash + ")"
      );
      $("#userCoverImage").css(
        "background-image",
        "url( http://localhost:8080/ipfs/" + App.user.coverhash + ")"
      );
    }
  },
  ...
  renderDweets: async () => {
    let totalDweets = await App.contracts.dwitter.methods
      .totalDweets()
      .call({ from: App.account });
    let dweetcard = $("#dweet");
    $("#dweet").remove();
    ...
    for (var i = totalDweets; i >= App.dweetsLoaded; i--) {
      try {
        let dweet = await App.contracts.dwitter.methods
          .getDweet(i)
          .call({ from: App.account });
        let author = await App.contracts.dwitter.methods
          .getUser(dweet.author)
          .call({ from: App.account });
        console.log(author);
        console.log(dweet);
        let dweeettemplate = dweetcard.clone();
        dweeettemplate
          .find(".fullname strong")
          .html(
            author.name +
              `<img src="/public/assets_index/img/tick.png" height="20" width="20">`
          );
        if (dweet.imghash != "")
          dweeettemplate
            .find(".tweet-text img")
            .attr("src", "http://localhost:8080/ipfs/" + dweet.imgHash);
        ...
        dweeettemplate
          .find(".tweet-card-avatar")
          .attr("src", "http://localhost:8080/ipfs/" + author.imghash);
  ...
  like: async (e) => {
    ...
    await App.contracts.dwitter.methods
      .likeDweet(parseInt(dweetId))
      .send({ from: App.account });
  },

  report: async (e) => {
    let dweetId = e.currentTarget.id;
    let price = await App.contracts.dwitter.methods
      .reportingstakePrice()
      .call({ from: App.account });
    console.log(price);
    await App.contracts.dwitter.methods
  ...
  showComments: async (e) => {
    ...
    let comments = await App.contracts.dwitter.methods
      .getDweetComments(dweetId)
      .call({ from: App.account });
    console.log(comments);
    if (comments.length == 0)
      $("#commentContainer")
        .html("<h3 class='mx-5'>There are no Comments!</h3>")
        .height("50px");
    else
      for (var i = 0; i <= comments.length - 1; i++) {
        let comment = await App.contracts.dwitter.methods
          .getComment(comments[i])
          .call({ from: App.account });
        let author = await App.contracts.dwitter.methods
        ...
        commentDiv
          .find(".title a")
          .html("<b>" + author.name + "</b> @" + author.username);
        commentDiv
          .find(".time")
          .text(new Date(comment.timestamp * 1000).toDateString());
    ...
    $(".commentBtn").on("click", async (e) => {
      ...
      await App.contracts.dwitter.methods
        .createComment(dweetId, $("#commentArea").val())
        .send({ from: App.account });
      $("#commentArea").val("");
  ...
  dweet: async () => {
    ...
    if (image) {
      ...
      reader1.onloadend = async function () {
        ...
        hash = result[0].hash;
        await App.contracts.dwitter.methods
          .createDweet($("#dweetTag").val(), $("#dweetContent").val(), hash)
          .send({ from: App.account });
  ...
  showAdvertisements: async () => {
    App.advertisementsList = await App.contracts.dwitter.methods
      .getAds()
      .call({ from: App.account });
    ...
    async function show() {
      if (App.noOfAds > 0) {
        ...
        let ad = await App.contracts.dwitter.methods
          .getAd(adindex)
          .call({ from: App.account });
        if (ad.status == 1 && Date.now() / 1000 < ad.expiry) {
          $("#ad").attr("href", ad.link);
          $("#ad img").attr("src", "http://localhost:8080/ipfs/" + ad.imgHash);
        } else {
  ...
  showAdvertisementStatus: async () => {
    ...
    let advertisementsList = await App.contracts.dwitter.methods
      .myAdvertisements()
      .call({ from: App.account });
    for (var i = 0; i < advertisementsList.length; i++) {
      let status = await App.contracts.dwitter.methods
        .getAdvertisementStatus(advertisementsList[i])
        .call({ from: App.account });
      if (status == 0) {
        let html =
          `      <tr>
  ...
  showReportStatus: async () => {
    ...
    $("#statusModalBody").empty();
    let reportsList = await App.contracts.dwitter.methods
      .myReportings()
      .call({ from: App.account });
    for (var i = 0; i < reportsList.length; i++) {
      let status = await App.contracts.dwitter.methods
        .getReportedDweetStatus(reportsList[i])
        .call({ from: App.account });
      if (status == 0) {
        let html =
          `      <tr>
        ...
        let userClaimStatus = await App.contracts.dwitter.methods
          .reportingClaimStatus(reportsList[i])
          .call({ from: App.account });
        if (userClaimStatus == 1) {
          let html =
            ...
            `</td>
            <td>Banned</td>
            <td><button type="button" class="btn-success claimReportReward" id="` +
            reportsList[i] +
            `">Claim</button></td>
            </tr>`;
          $("#statusModalBody").append(html);
  ...
  showFakeReportingReward: async () => {
    ...
    let reward = await App.contracts.dwitter.methods
      .fakeReportingSuitReward()
      .call({ from: App.account });
