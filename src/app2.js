App = {
  loading: false,
  ipfs: window.IpfsApi("127.0.0.1", 5001, { protocol: "https" }),
  contracts: {},
  load: async () => {
    await App.loadWeb3();
  ...
  checkUser: async () => {
    let isMaintainer = await App.contracts.dwitter.methods
      .isMaintainer(App.account)
    ...
    if (!isMaintainer) {
      ...
      $("#generalMsgModal").modal("show");
      $("#generalModalMessage").text(
        "You are not a Maintainer or Owner, Access Denied!"
      );
    }
  },
  render: async () => {
  ...
  renderData: async () => {
    let totalDweets = await App.contracts.dwitter.methods
      .totalDweets()
      .call({ from: App.account });
    let totalUsers = await App.contracts.dwitter.methods
      .totalUsers()
      .call({ from: App.account });
  ...
  renderReports: async () => {
    try {
      ...
      for (var i = 0; i < ReportsList.length; i++) {
        ...
        if (status == 0) {
          let html =
            ...
            ReportsList[i] +
            `</td>
         <td><button type="button" class="btn-warning viewdweet" id="` +
            ReportsList[i] +
            `">View</button></td>
         <td>
            <button type="button" class="btn-success ban" id="` +
            ReportsList[i] +
            `">Ban</button>
            <button type="button" class="btn-danger free" id="` +
            ReportsList[i] +
            `">Free</button>
         </td></tr>`;
      ...
      $("#generalMsgModal").modal("show");
      $("#generalModalMessage").text(
        "Error! You are not a user of this Platform! Please Register to Continue"
      );
      App.setLoading(true);
      // App.load();
    }
    ...
    $(".viewdweet").on("click", async (e) => {
      ...
      $("#dweetImage").attr(
        "src",
        "http://localhost:8080/ipfs/" + dweet.imgHash
      );
  ...
  renderAdvertisements: async () => {
      ...
      for (var i = 0; i < advertisementsList.length; i++) {
        ...
        if (status == 0) {
          let html =
            ...
            `</td>
       <td><button type="button" class="btn-warning viewad" id="` +
            advertisementsList[i] +
            `">View</button></td>
       <td>
          <button type="button" class="btn-success accept" id="` +
            advertisementsList[i] +
            `">Accept</button>
          <button type="button" class="btn-danger reject" id="` +
            advertisementsList[i] +
            `">Reject</button>
       </td></tr>`;
    ...
    $(".accept").on("click", async (e) => {
      ...
      await App.contracts.dwitter.methods
        .advertisementApproval(adId, true)
        .send({ from: App.account });
    });
    ...
    $(".reject").on("click", async (e) => {
      let adId = e.currentTarget.id;
      await App.contracts.dwitter.methods
        .advertisementApproval(adId, false)
        .send({ from: App.account });
    });
    ...
    $(".viewad").on("click", async (e) => {
      ...
      $("#adLink").text(ad.link);
      $("#adImage").attr("src", "http://localhost:8080/ipfs/" + ad.imgHash);
    });
  },
  setLoading: (boolean) => {
    App.loading = boolean;
  ...
  maintainerSettings: async () => {
    let owner = await App.contracts.dwitter.methods
      .owner()
      .call({ from: App.account });
    if (App.account.toLowerCase() == owner.toLowerCase()) {
      ...
      $("#removeMaintainerBtn").on("click", async () => {
        let address = $("#maintainerAddress").val();
        await App.contracts.dwitter.methods
          .revokeMaintainer(address)
          .send({ from: App.account });
        $("#MaintainerModalMsg").text(
          "Success, Maintainer Removed Successfully"
        );
      });
    } else {
      $("#generalMsgModal").modal("show");
      $("#generalModalMessage").text(
        "Access Denied!!! You are not Owner of this Platform"
      );
    }
  },
  ...
  withdrawContractFunds: async () => {
    ...
    if (App.account.toLowerCase() == owner.toLowerCase()) {
      ...
      let balance = await App.contracts.dwitter.methods
        .getBalance()
        .call({ from: App.account });
      ...
      $("#WithdrawContractBalance").on("click", async () => {
        ...
        await App.contracts.dwitter.methods
          .transferContractBalance(amount)
          .send({ from: App.account });
        let balance = await App.contracts.dwitter.methods
          .getBalance()
          .call({ from: App.account });
